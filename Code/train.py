...
from DataDeal.Data_loader import preped_RN_data, SIDD_val, DND, DEBRIS_test
...
def train():
    ...
    # --- 修改 Train 加载逻辑 ---
    if 'DEBRIS' in args.config:
        # 对应 DEBRIS.yaml 中的路径 prep/DEBRIS_Train
        train_dataset_path = os.path.join(cfg['data_root_dir'], 'prep/DEBRIS_Train')
        # 你的训练数据是预处理好的 RN 数据，所以用 preped_RN_data
    ...
    # --- 修改 Val 加载逻辑 ---
    if 'DEBRIS' in args.config:
        # 对应 DEBRIS.yaml 中的路径 DEBRIS/test_dataset
        val_dataset_path = os.path.join(cfg['data_root_dir'], 'DEBRIS/test_dataset')
        # 使用新写的类
        val_dataset = DEBRIS_test(**val_args, dataset_path=val_dataset_path)
    else:
        # 原有的 SIDD 逻辑
