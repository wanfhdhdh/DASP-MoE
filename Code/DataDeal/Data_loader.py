...
class DEBRIS_test(RealDataSet):
    '''
    DEBRIS dataset class for validation/testing (Noisy only)
    '''
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _load_data(self, data_idx):
        ...
        # 加载图像
        noisy_img = self._load_img(img_path)
        
        # 因为没有 Clean GT，只返回 real_noisy
        # 为了保存图片时能知道文件名，建议把路径信息也放入
        return {'real_noisy': noisy_img, 'instances': img_path}
